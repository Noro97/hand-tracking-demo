...
**Дорожная карта шутера (`docs/SHOOTER-ROADMAP.md`, 14.08.2026):** план из 6 фаз + готовый промпт для кодового агента, как довести прототип 025 до полноценной игры с текстурами/звуком/жестовым UI. **Критический блокер зафиксирован: игровой канвас наследует глобальное зеркало `scaleX(-1)`** — любая текстура с текстом или асимметрией отрисуется задом наперёд, поэтому фаза 0.1 (снять зеркало со слоя, зеркалить координаты) обязана быть первой. Второй фундамент — отвязать симуляцию/рендер от темпа камеры на rAF. Вопрос происхождения ассетов оставлен пользователю.
