...
**Задача 018 (clinician dashboard foundation) — ЗАБЛОКИРОВАНА внешней Supabase-квотой.** Код полностью написан (Supabase-схема+RLS, auth, react-router, dashboard, patient detail) на ветке `claude/clinician-dashboard-foundation`, `tsc`/`test`/`build` зелёные — но живая проверка (signup/login/patient-CRUD) не пройдена: организация `selfish-harlequin-zdqlfrw` (free plan) уже над квотой БД (из-за проекта "Proof", см. `.tasks/RETRO.md` и глобальную память) — квота, похоже, применяется на уровне ОРГАНИЗАЦИИ, блокируя данные-API даже в новом, почти пустом `hand-tracking-demo` Supabase-проекте (`slrgjenpvsuxvhqqxwrp`). Ждёт решения пользователя: сброс квоты/апгрейд/чистка Proof/новая организация. PR не открыт (task помечена `blocked`, не имеет смысла открывать PR на неподтверждённый живой функционал).
