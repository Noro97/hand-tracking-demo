...
**PR #1 смержен в `master`** (2026-07-06) — весь BBT-функционал (задачи 013–017) теперь в trunk.

**Задача 018 (clinician dashboard foundation) — ПРИОСТАНОВЛЕНА, ждёт пользователя.** Код полностью написан (Supabase-схема+RLS, auth, react-router, dashboard, patient detail) на ветке `claude/clinician-dashboard-foundation` (запушена, PR не открыт), `tsc`/`test`/`build` зелёные — но живая проверка (signup/login/patient-CRUD) заблокирована: организация `selfish-harlequin-zdqlfrw` (free plan) уже над квотой БД (из-за проекта "Proof") — квота применяется на уровне ОРГАНИЗАЦИИ, блокируя дата-API даже в новом, почти пустом `hand-tracking-demo` Supabase-проекте (`slrgjenpvsuxvhqqxwrp`). **Пользователь решил: ждать ежемесячного сброса квоты или Pro-апгрейд** — не переключаться на новую организацию, не трогать Proof, не продолжать без верификации. Следующая сессия начинается с этой ветки, когда пользователь подтвердит, что квота снята.
