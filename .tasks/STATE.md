...
## Текущий фокус
Пользователь подтвердил направление: **medtech B2B SaaS для реабилитации кисти** (`docs/STRATEGY.md` §6/§8, направление 4.1a) — официальная главная цель проекта, вопрос "что строим" закрыт. Задачи 013+014 реализовали и довели до рабочего состояния первый технический шаг MVP-плана §7 (недели 3-4): Box-and-Block-Test измерительный цикл (`features/bbtSession.ts`, `hooks/useBBTSession.ts`, `components/BBTSessionDemo.tsx`) — теперь единственный экран приложения (Draw Demo удалён в 014), с выбором тестируемой руки перед стартом и толерантностью к MediaPipe handedness-relabel. PR #1 открыт на review, ещё не смержен — реальная проверка на живой камере (толерантность relabel, надёжность single-hand gating) пользователем ещё не подтверждена. Следующие шаги плана §7: неделя 5 (clinician-dashboard со списком пациентов и историей сессий) — следующая техническая задача; недели 1-2 (найти 2-3 clinical advisors) — не техническая, за пользователем.
