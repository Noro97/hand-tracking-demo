...
**Задача 024 (`claude/two-hand-screen`, поверх 023): AnimeGANv2 — настоящая AI-стилизация в браузере.** Пользователь спросил, какой бесплатной AI-технологией сделать «мультяшность» на JS. Честно сказано, что его референс — оффлайн-диффузия, не live-фильтр; из 4 вариантов выбран AnimeGANv2 (ONNX, 8.2 МБ, бесплатно, полностью client-side). Сигнатура модели выяснена эмпирически (NHWC, tanh, динамические размеры). Инференс в Web Worker, синхронный `stylize` отдаёт последний готовый кадр. **Замерено в реальном браузере: WebGPU 49 мс (~20 fps), WASM-фоллбэк 342 мс (~3 fps).** Существующий буфер 192×108 оказался идеальным входом модели. Два бага, найденных пользователем (Vite `public/` импорт; `optimizeDeps` ломает `initWasm`), исправлены; урок записан в RETRO. 71 тест. Требуется `npm run fetch-model` (модель gitignored).

**Задача 025 (`claude/two-hand-screen`): игра-стрелялка на жесте «пистолет».** Указательный целится, падение большого пальца на кулак стреляет. Отдельный экран «Shooting» (клинический BBT не тронут). Поза — scale-invariant отношения разгибания пальцев (`lib/pistolPose.ts`); спуск переиспользует `PinchTracker` (гистерезис+дебаунс уже ровно то, что нужно триггеру) + edge-detection = один выстрел на нажатие; прицел — hitscan-луч из кончика пальца (`lib/aiming.ts`), выбирается ближайшая мишень. Clock+RNG инжектируются → 31 новый тест. Рендер на отдельном stacked-канвасе, движок не менялся. Браузерный прогон поймал баг, который юнит-тесты маскировали (спавн зависел от величины `Date.now()`) — исправлено, урок в RETRO. 115 тестов.
