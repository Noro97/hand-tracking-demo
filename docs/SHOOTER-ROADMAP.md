...
**Готовые промпты для генерации — `docs/ASSET-PROMPTS.md`** (стиль-якорь, промпт на каждый ассет, пайплайн). Ключевое оттуда: неон на чёрном фоне рисуется аддитивно (`globalCompositeOperation = 'lighter'`), поэтому альфа-канал и удаление фона не нужны вовсе; кадры анимации не генерируются — один статичный спрайт, анимация кодом.
